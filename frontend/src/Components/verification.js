...
const Verification = () => {
    ...
    const fetchOTP = async () => {
        try {
            const response = await axios.get('http://localhost:5001/getotp', {});
    ...
    const sendOTP = async () => {
        ...
        try {
            ...
            const response = await axios.post('http://localhost:5001/genotp', {
                userId: 
                email,
            });
            console.log("hhhhhhhh")
    ...
    const handleSubmit = async () => {
        try {
            if (generatedOTP !== null) {
                if (otp === generatedOTP) {
                    axios
                        .post('http://localhost:5001/signup', { uname: username, mail: email, key: password })
                        .then((response) => {
